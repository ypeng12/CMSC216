 ...
 *
 * Tests calling make_target() when the target does not exist (the action
 * should be performed).
 *
...
int main(void) {
  ...
  fakefile= read_fakefile("public09.fakefile");
  ...
  assert(make_target(fakefile, "nonexistent-file") == 0);
